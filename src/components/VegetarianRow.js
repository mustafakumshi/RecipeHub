...
const VegetarianRow = () => {
  ...
  const getCuisinev = async () => {
    ...
    if (check) {
      ...
      const api = await fetch(
        `https://api.spoonacular.com/recipes/random?apiKey=3dd60be334c34cf4859f7887eeb74ea0&number=9&tags=vegetarian`);
  ...
  const getCuisinenon = async () => {
    ...
    if (check) {
      ...
      const api = await fetch(
        `https://api.spoonacular.com/recipes/random?apiKey=3dd60be334c34cf4859f7887eeb74ea0&number=9`);
      const data = await api.json();
